...
def _flex_state_planner_summary(state: dict[str, Any] | None) -> str:
    ...
    HAZARD: tools.py 有一份更完整的同名函数（含 row_hint / artifact_note）。
    job_tools.collect 走这里，同步 sub_agent_tool 走胖版，两条路径摘要不一致。
    """
...
def _parse_job_subagent_completed(
    *,
    ...
    if isinstance(worker_result_payload, dict):
        ...
        assistant_reply = (
            str(parsed.get("assistant_reply") or parsed.get("frontend_msg") or "").strip()
            or worker_result.final_answer
            or _flex_state_planner_summary(flex_state)
        )
        worker_payload = worker_result.to_dict()
        if assistant_reply and not str(worker_payload.get("final_answer") or "").strip():
            worker_payload = dict(worker_payload)
            worker_payload["final_answer"] = assistant_reply
        return JobSubagentOutcome(
            original_msg=worker_payload,
    ...
    if not assistant_reply:
        assistant_reply = _flex_state_planner_summary(flex_state)
