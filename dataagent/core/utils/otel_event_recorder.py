...
# Sub-agent live spans hop to the parent SSE via this stderr prefix (no prompt/args).
OTEL_LIVE_STDERR_PREFIX = "DATAAGENT_OTEL "
...
class OtelEventRecorder:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            live_stderr: When True, also print compact spans to stderr for the
                parent process to forward onto SSE. Default False so unit tests
                and CLI constructors do not spam the terminal.
        """
        ...
        self._live_stderr = bool(live_stderr)
    ...
    def from_state(cls, state: dict[str, Any]) -> OtelEventRecorder | None:
        ...
        return cls(
            ...
            sub_id=sub_id,
            run_id=run_id,
            live_stderr=role == "sub-agent" and not bool(state.get("terminal_mode")),
        )
    ...
    def record_llm_start(self, model: str, messages: Any = None) -> None:
        ...
        with self._lock:
            ...
            self._llm_name = str(model or "")
        self._emit_live({"kind": "llm", "phase": "start", "name": str(model or ""), "timestamp": now})
    ...
    def record_llm_end(
        ...
        with self._lock:
            self._events.append(event)
            started = self._llm_started_at
            name = self._llm_name
            self._llm_started_at = None
        live: dict[str, Any] = {"kind": "llm", "phase": "end", "name": name, "timestamp": now}
        if started is not None:
            live["duration_ms"] = max(0, int((now - started) * 1000))
        if usage_dict is not None:
            live["usage"] = usage_dict
    ...
    def record_tool_start(
        ...
        with self._lock:
            self._events.append(event)
            self._tool_started_at[tool_call_id] = now
            self._tool_names[tool_call_id] = str(tool_name or "")
        self._emit_live(
            {
                "kind": "tool",
                "phase": "start",
    ...
    def _emit_live(self, payload: dict[str, Any]) -> None:
        """Best-effort compact span for SSE. Never include prompt/args/content."""
        event: dict[str, Any] = {"type": "otel_span"}
        event.update(payload)
        if self._sub_id > 0:
            event["sub_id"] = self._sub_id
        parent_tool = self._parent_tool_call_id
        if parent_tool:
            event["parent_tool_call_id"] = parent_tool
        try:
            ...
            pass
        if not self._live_stderr:
            return
        try:
            encoded = json.dumps(event, ensure_ascii=False, separators=(",", ":"))
            sys.stderr.write(f"{OTEL_LIVE_STDERR_PREFIX}{encoded}\n")
            sys.stderr.flush()
        except Exception:
            pass
