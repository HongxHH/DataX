...
from dataagent.core.flex.progress_hint import (
    context_rewrite_event as _context_rewrite_event,
    cross_session_recall_event as _cross_session_recall_event,
    progress_hint_event as _progress_hint_event,
)
from dataagent.core.flex.utils.hitl_config import is_human_feedback_enabled
...
_AGENT_PRE_HOOK_HINTS: tuple[tuple[str, str], ...] = (
    ("session_history_restore", "正在恢复会话历史…"),
    ("cross_session_recall", "正在检索跨会话记忆…"),
    ("context_reference_rewriter", "正在解析问题中的指代（可能调用模型）…"),
...
def _hint_for_agent_pre_hook(hook: Any) -> str:
    ...
    for key, hint in _AGENT_PRE_HOOK_HINTS:
        if key in name:
            return hint
...
def _sync_enable_human_feedback(state: dict[str, Any], config: Mapping[str, Any] | None) -> None:
    """Fill HITL enablement from YAML when the caller did not set it (kernel setdefault)."""
    cfg = dict(config) if isinstance(config, Mapping) else {}
    state.setdefault("enable_human_feedback", is_human_feedback_enabled(cfg))


def _sync_enable_portrait(state: dict[str, Any], config: Mapping[str, Any] | None) -> None:
    """Fill portrait enablement from YAML when the caller did not set it.

    Shell ``astream`` does not pass ``enable_portrait``. Without this, portraiter
    skips snapshot/MEMORY.md writes and the next session's cross-session recall
    stays empty even when ``AGENT_CONFIG.enable_portrait`` is true.
    """
    cfg = dict(config) if isinstance(config, Mapping) else {}
    agent_config = cfg.get("AGENT_CONFIG", {})
    if not isinstance(agent_config, Mapping):
        agent_config = {}
    state.setdefault("enable_portrait", bool(agent_config.get("enable_portrait", False)))


async def _emit_context_rewrite(state: Any) -> AsyncGenerator[Any, None]:
    event = _context_rewrite_event(state)
    if event is None:
        return
    yield event
    await _flush_stream()
...
async def _emit_cross_session_recall(state: Any) -> AsyncGenerator[Any, None]:
    event = _cross_session_recall_event(state)
    if event is None:
        return
    yield event
    await _flush_stream()
...
class FlexAgent(BaseAgent):
    ...
    def __init__(
        ...
        *,
        ...
        enable_hitl = is_human_feedback_enabled(config or {})
        if enable_hitl:
    ...
    async def chat(
        ...
        **kwargs,
        ...
        enable_hitl = is_human_feedback_enabled(self.config)
        # chat 接口下 terminal_mode 默认 True
        terminal_mode = agent_config.get("terminal_mode", True)

        _sync_enable_human_feedback(initial_state, self.config)
        _sync_enable_portrait(initial_state, self.config)
    ...
    async def _astream_langgraph(self, **kwargs: Any) -> AsyncGenerator[Any, None]:
        ...
        runtime = self._create_call_runtime()
        yield _progress_hint_event("正在初始化会话上下文…")
        await _flush_stream()
        ...
        with self._performance_run(
            ...
            async for event in self._emit_pre_hook_progress(context_state, runtime, done):
                yield event
            context_state = done[0] if done else context_state
            async for event in _emit_cross_session_recall(context_state):
                yield event
            async for event in _emit_context_rewrite(context_state):
                yield event
            initial_state_for_persist = context_state
            ...
            yield _progress_hint_event("正在启动规划节点…")
            await _flush_stream()
    ...
    async def _astream_openjiuwen(self, *args: Any, **kwargs: Any) -> AsyncGenerator[Any, None]:
        ...
        _sync_enable_human_feedback(initial_state, self.config)
        _sync_enable_portrait(initial_state, self.config)
        initial_state.setdefault("terminal_mode", bool(agent_config.get("terminal_mode", False)))
        ...
        with self._performance_run(
            ...
            async for event in self._emit_pre_hook_progress(initial_state, runtime, done):
                yield event
            initial_state = dict(done[0] if done else initial_state)
            async for event in _emit_cross_session_recall(initial_state):
                yield event
            async for event in _emit_context_rewrite(initial_state):
                yield event
    ...
    def _apply_agent_pre_hooks_with_progress(
    ...
    ) -> Generator[tuple[str, dict[str, Any]], None, dict[str, Any]]:
        """先 yield 进度，再执行对应 hook。

        chat() 与单测走这条同步路径；astream 走 ``_emit_pre_hook_progress``。
        改 hook 顺序时两处必须一起改。
        """
        ...
        for hook in getattr(self, "_builtin_agent_pre_hooks", None) or []:
            ...
            current = self._invoke_agent_pre_hook(hook, current, runtime, yaml_hook=False)
        for hook in getattr(self, "_pre_hooks", None) or []:
            yield _progress_hint_event(_hint_for_agent_pre_hook(hook))
            current = self._invoke_agent_pre_hook(hook, current, runtime, yaml_hook=True)
        return current

    async def _emit_pre_hook_progress(
        self,
        state: dict[str, Any],
        runtime: Any,
        done: list[dict[str, Any]],
    ...
    def _run_agent_pre_hooks(self, state: dict[str, Any], runtime: Any) -> dict[str, Any]:
        ...
        chat() 必须走 ``_run_builtin_agent_pre_hooks``，以便测试/子类替换该方法。
        """
        current_state = self._run_builtin_agent_pre_hooks(state, runtime)
        for hook in getattr(self, "_pre_hooks", None) or []:
            current_state = self._invoke_agent_pre_hook(hook, current_state, runtime, yaml_hook=True)
    ...
    def _dispatch_agent_post_hooks(
        self,
        state: dict[str, Any],
        runtime: Any,
        *,
        ...
        """Run post-hooks; astream must not wait on LLM portraiter.

        Copilot keeps the SSE open until ``astream`` finishes. Portraiter does two
        blocking LLM calls, so awaiting it here freezes the UI after the answer
        is already on screen. ``chat()`` still blocks because the process may exit.
        """
        if not self._post_hooks or not state:
            return

        def _run() -> None:
            try:
                self._run_agent_post_hooks(state, runtime)
            except Exception as e:
    ...
    async def _finalize_context_after_stream(
        ...
        *,
        ...
        # post-hooks：图终态为主；enable_portrait 从入口 state 带回，避免 astream values 丢开关。
        # 不阻塞 SSE：portraiter 的 LLM 调用与用户可见收尾解耦。
        if self._post_hooks and final_state:
            logger.debug(f"[FlexAgent] Scheduling {len(self._post_hooks)} post-hooks")
            logger.debug(
                "[FlexAgent] post-hook state keys: {} messages_count={}",
                list(final_state.keys()),
                len(final_state.get("messages") or []),
            )
            hook_state = dict(final_state)
            if isinstance(initial_state, dict) and bool(initial_state.get("enable_portrait")):
                hook_state["enable_portrait"] = True
            self._dispatch_agent_post_hooks(hook_state, runtime, blocking=False)
    ...
    def _prepare_context_for_langgraph_stream(self, input_val: dict[str, Any], runtime: Any = None) -> None:
        ...
        try:
            ...
            _sync_enable_human_feedback(input_val, self.config)
            _sync_enable_portrait(input_val, self.config)
    ...
    def _run_builtin_agent_pre_hooks(self, state: dict[str, Any], runtime: Any = None) -> dict[str, Any]:
        ...
        s = state
        for hook in getattr(self, "_builtin_agent_pre_hooks", None) or []:
            s = self._invoke_agent_pre_hook(hook, s, runtime, yaml_hook=False)
        return s
