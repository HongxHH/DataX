...
from dataagent.core.flex.context_usage import context_usage_from_runtime, emit_context_usage
from dataagent.core.flex.prompt_inventory import emit_prompt_inventory, prompt_inventory_from_messages
from dataagent.core.flex.progress_hint import progress_hint_payload
...
class Planner(BaseNode):
    ...
    async def _aprocess(self, state: FlexState, runtime: Any = None) -> dict[str, Any] | FlexState:
        ...
        if runtime.get_config("CONTEXT.enable_profiling", False):
            context.profiling()
        await self._emit_progress_hint(writer, "正在同步上下文任务…")
        ...
        try:
            ...
            await self._emit_progress_hint(writer, "正在等待规划模型返回…")
    ...
    async def _emit_progress_hint(self, writer, hint: str) -> None:
        writer(progress_hint_payload(hint, node=self.name))
