...
class GeneratorNode(BaseNL2SQLNode):
    ...
    async def generate_with_llm(
        self, strategy: str, settings: dict, context: dict, *, emit_draft: bool = False
    ):
        ...
        content = await self._stream_llm(prompts, emit_draft=emit_draft)
    ...
    async def strategy_prompt(self, settings, context, *, emit_draft: bool = False):
        """Generate prompt-strategy candidates."""
        settings["num_samples"] = self.num_samples
        return await self.generate_with_llm("prompt", settings, context, emit_draft=emit_draft)

    async def strategy_skeleton(self, settings, context, *, emit_draft: bool = False):
        """Generate skeleton-strategy candidates."""
        return await self.generate_with_llm("skeleton", settings, context, emit_draft=emit_draft)

    async def strategy_icl(self, settings, context, *, emit_draft: bool = False):
        """Generate in-context-learning candidates."""
        return await self.generate_with_llm("icl", settings, context, emit_draft=emit_draft)

    async def strategy_dc(self, settings, context, *, emit_draft: bool = False):
        """Generate divide-and-conquer candidates."""
        return await self.generate_with_llm("dc", settings, context, emit_draft=emit_draft)

    async def run_strategy(self, strategy, settings, context, *, emit_draft: bool = False):
        """Run one configured generation strategy."""
        fn = getattr(self, f"strategy_{strategy}", None)
        if fn is None:
            raise ValueError(f"Unknown strategy: {strategy}")
        return await fn(settings, context, emit_draft=emit_draft)
    ...
    async def _aprocess(self, state: NL2SQLState, runtime: Any = None) -> NL2SQLState:
        ...
        for strategy in self.strategies:
            for _ in range(self.num_workers):
                tasks.append(
                    asyncio.create_task(
                        self.run_strategy(strategy, settings.copy(), context, emit_draft=emit_draft)
                    )
                )
                emit_draft = False
