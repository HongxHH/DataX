    ...
    *,
    ...
    for scope in traverse_scope(statement):
        for source in scope.sources.values():
            ...
            if not matches:
                message = (
                    "Source table is not in the provided semantic schema: "
                    f"{source_sql}. Use a table that appears in the provided semantic schema."
                )
                return [SecurityViolation("SCHEMA-001", message)]
            if len(matches) != 1:
                message = (
                    "Source table cannot be resolved uniquely in the provided semantic schema: "
                    f"{source_sql}. Use exactly one modeled table and check its "
