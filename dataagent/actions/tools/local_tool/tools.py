...
from dataagent.utils.runtime_paths import (
    SUBAGENT_OUTPUT_DIR_ENV,
    dataagent_package_root,
...
def _flex_state_row_count_hint(state: dict[str, Any]) -> str:
    """Tell the parent planner whether the full result is already available."""
    rows = state.get("rows")
    preview = state.get("rows_preview")
    if isinstance(rows, list) and rows:
        total = len(rows)
        preview_n = len(preview) if isinstance(preview, list) else total
        ...
        return ""
    if total <= preview_n:
        return f"查询共 {total} 行，结果完整，无需再次调用 sub_agent_tool 补全。"
    return (
        f"查询共 {total} 行，预览展示 {preview_n} 行；"
        "完整结果已在子 Agent 中返回，禁止为补全预览再次调用 sub_agent_tool。"
    )


def _flex_state_artifact_note(state: dict[str, Any]) -> str:
    """Planner-visible absolute paths for CSV / SQL / 摘录 / 图 / 报告."""
    bits: list[str] = []
    csv_path = str(state.get("csv_path") or "").strip()
    sql_path = str(state.get("sql_path") or "").strip()
    if csv_path:
        bits.append(f"CSV 已落盘：{csv_path}")
    if sql_path:
        bits.append(f"SQL 已落盘：{sql_path}")
    persist_count = state.get("persist_row_count")
    if isinstance(persist_count, int) and csv_path:
        extra = "（已截断到上限 10000 行）" if state.get("persist_truncated") else ""
        bits.append(f"落盘 {persist_count} 行{extra}；预览约 50 行，二者分开。")
    recall_path = str(state.get("recall_path") or "").strip()
    if recall_path:
        bits.append(f"文档摘录：{recall_path}")
    image_path = str(state.get("image_path") or "").strip()
    if image_path:
        bits.append(f"图表：{image_path}")
    report_path = str(state.get("report_path") or "").strip()
    if report_path:
        bits.append(f"报告：{report_path}")
    return "\n".join(bits)


_NL2SQL_NON_SQL_QUERY = re.compile(
    r"(?:运行|执行)\s*(?:指定路径)?(?:的)?(?:本地)?(?:python|Python)"
    r"|python\s*脚本"
    r"|matplotlib"
    r"|plt\.figure",
    re.IGNORECASE,
...
def _agent_yaml_type(cfg_path: Path) -> str:
    """Return ``AGENT_CONFIG.type`` from a sub-agent YAML, or empty on read/parse failure."""
    try:
        payload = yaml.safe_load(cfg_path.read_text(encoding="utf-8")) or {}
    except (OSError, yaml.YAMLError):
        return ""
    agent = payload.get("AGENT_CONFIG") if isinstance(payload, dict) else None
    if not isinstance(agent, dict):
        return ""
    return str(agent.get("type") or "").strip().lower()


def _nl2sql_non_sql_query_message(query: str, cfg_path: Path) -> str | None:
    """Reject plot/script tasks that were mistakenly sent to an NL2SQL worker."""
    if _agent_yaml_type(cfg_path) != "nl2sql":
        return None
    if not _NL2SQL_NON_SQL_QUERY.search(str(query or "")):
        return None
    return (
        "这次委派的是 NL2SQL，它只能做只读 SQL 查询，不能运行 Python 或画图。"
        "请改用 Landcheck Plot（landcheck_plot.yaml），"
        "在 query 中写入已落盘 CSV 与 SQL 的绝对路径（src_data_path / sql_command_path）。"
    )


def _flex_state_planner_summary(state: dict[str, Any] | None) -> str:
    """Build a short planner-visible summary from NL2SQL / Flex final state."""
    if not isinstance(state, dict):
        return ""
    stream = str(state.get("stream_message") or "").strip()
    row_hint = _flex_state_row_count_hint(state)
    artifact_note = _flex_state_artifact_note(state)
    extras = [part for part in (row_hint, artifact_note) if part and part not in stream]
    if stream:
        if extras:
            return stream + "\n\n" + "\n".join(extras)
        return stream
    ...
    rows = state.get("rows")
    if sql and isinstance(rows, list) and rows:
        col_names = [str(c) for c in columns] if isinstance(columns, list) else []
        preview_rows = rows[:10]
        lines = [f"SQL: {sql}"]
        if col_names:
            lines.append(f"列: {', '.join(col_names)}")
        lines.append(f"结果预览（共 {len(rows)} 行）: {preview_rows}")
        lines.extend(extras)
        return "\n".join(lines)
    if sql:
        parts = [f"SQL: {sql}", *extras]
...
_ARTIFACT_PATH_RE = re.compile(
    r"(?:[A-Za-z]:[\\/]|/)[^\s`'\"<>|]+\.(?:csv|sql|png|json|md)",
    re.IGNORECASE,
)
_PROMOTE_STATE_KEYS = (
    "csv_path",
...
_PROMOTE_DIR_NAMES = frozenset({"nl2sql", "document_recall", "plot", "report"})


def _parent_shared_output_dir() -> Path | None:
    try:
    ...
    if root is None:
        return None
    shared = Path(root) / "subagent_output"
    try:
...
def _promote_file_into_shared(src: Path, shared: Path) -> Path | None:
    if not src.is_file():
        return None
    rel_name = src.name
    for parent in src.parents:
        if parent.name in _PROMOTE_DIR_NAMES:
            try:
                rel_name = str(src.relative_to(parent.parent)).replace("\\", "/")
            except ValueError:
                rel_name = src.name
            break
    dest = shared / rel_name
    try:
        dest.parent.mkdir(parents=True, exist_ok=True)
        if src.resolve() != dest.resolve():
            shutil.copy2(src, dest)
    except OSError as exc:
        logger.warning("subagent artifact promote failed {} -> {}: {}", src, dest, exc)
        return None
    return dest


def _load_recall_excerpts(path: Path) -> tuple[list[dict[str, Any]], str]:
    try:
    ...
    if isinstance(images, list):
        for item in images:
            if isinstance(item, dict):
                _add(item.get("image_path") or item.get("path"))
            else:
                _add(item)
...
def _promote_subagent_artifacts_into_parent(
    ...
    for src in _collect_candidate_artifact_paths(promoted, assistant_reply):
        ...
        if suffix == ".csv":
            ...
            excerpts, summary = _load_recall_excerpts(dest)
            promoted["recall_path"] = dest_s
            if excerpts:
...
def _subagent_outcome_to_public_tool_dict(outcome: _SubagentCompletedOutcome) -> dict[str, Any]:
    ...
    if artifact_note and artifact_note not in assistant_reply:
...
async def _sub_agent_run_subprocess_and_collect_outcome(
    *,
    ...
    try:
        ...
        shared_output = _parent_shared_output_dir()
        if shared_output is not None:
            env[SUBAGENT_OUTPUT_DIR_ENV] = str(shared_output)
        cmd = [
            sys.executable,
            "-m",
...
async def sub_agent_tool(
    ...
    rejected = _nl2sql_non_sql_query_message(query, cfg_path)
    if rejected:
        wr = _synthetic_worker_result_dict(
            sub_id=worker_sub_id,
            parent_session_id=resolved_session_id,
...
def _prepare_worker_initial_state_file(
    *,
    ...
    parent_workspace = path_kwargs.get("parent_workspace")
    if parent_workspace:
        ...
        payload["workspace"] = str(worker_root)
