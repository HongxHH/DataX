...
def _enrich_collect_subagent_for_planner(payload: dict[str, Any]) -> dict[str, Any]:
    """Ensure collect payloads expose NL2SQL rows/SQL via ``original_msg.final_answer``."""
    from dataagent.core.agents.subagent_subprocess_runner import _flex_state_planner_summary

    if not isinstance(payload, dict):
        return payload
    state = payload.get("state")
    summary = str(payload.get("summary") or payload.get("frontend_msg") or "").strip()
    if not summary and isinstance(state, dict):
        summary = _flex_state_planner_summary(state)
    if not summary:
        return payload
    enriched = dict(payload)
    enriched["summary"] = summary
    enriched["frontend_msg"] = summary
    original = enriched.get("original_msg")
    if isinstance(original, dict) and not str(original.get("final_answer") or "").strip():
        original = dict(original)
        original["final_answer"] = summary
        enriched["original_msg"] = original
    return enriched
...
def collect_subagent(job_id: str, *, _tool_context: ToolExecutionContext) -> dict[str, Any]:
    ...
    if kind_error is not None:
        return kind_error
    payload = agent_service.collect(job_id=normalized_job_id)
    if isinstance(payload, dict):
        return _enrich_collect_subagent_for_planner(payload)
    return payload
